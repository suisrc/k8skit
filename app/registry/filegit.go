...
func ExtractTgzByGit(outDir, preDir, rawURL string) error {
	...
	if tagName == "" {
		return errors.New("parser url error, no tag name")
	}
	// 提取用户基本信息
	...
	if err != nil {
		return errors.New("git clone error, " + err.Error())
	}
	// 获取标签对应的提交树
	head, err := repo.Head()
	if err != nil {
		return errors.New("get head error, " + err.Error())
	}
	commit, err := repo.CommitObject(head.Hash())
	if err != nil {
		return errors.New("get commit error, " + err.Error())
	}
	tree, err := commit.Tree()
	if err != nil {
		return errors.New("get tree error, " + err.Error())
	}
...
func CreateTgzByGit(srcDir, preDir, rawURL string) (tarball.Opener, error) {
	parsedURL, err := url.Parse(rawURL)
	if err != nil {
		return nil, err
	}
	...
	return func() (io.ReadCloser, error) {
		...
		go func() {
			...
			err := tree.Files().ForEach(func(file *object.File) error {
				...
				fileReader, err := file.Reader()
				if err != nil {
					return fmt.Errorf("read file %s error: %w", file.Name, err)
				}
				defer fileReader.Close()
				// 构造tar头信息
				fileMode, err := file.Mode.ToOSFileMode()
				...
				if err := tw.WriteHeader(header); err != nil {
					return fmt.Errorf("write tar header %s error: %w", file.Name, err)
				}
				// 写入文件内容
				if _, err := io.Copy(tw, fileReader); err != nil {
					return fmt.Errorf("write file content %s error: %w", file.Name, err)
				}
				return nil
			})
			if err != nil {
...
func CreateTgzFileByGit(outFile, srcDir, preDir, rawURL string) error {
	opener, err := CreateTgzByGit(srcDir, preDir, rawURL)
	if err != nil {
		return err
	}
