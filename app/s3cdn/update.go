...
func UploadToS3(hfs http.FileSystem, fim map[string]fs.FileInfo, ffc *front2.Config, cfg *Config, app, ver string) error {
	...
	if !cfg.Rewrite {
		...
		// if err == nil {
		// 	z.Logn("[_cdnskip]: upload to s3:", name, ", exists")
		// 	return nil
		// }
		// if minio.ToErrorResponse(err).Code != "NoSuchKey" {
		// 	return err
		// }
		// z.Logn("[_cdnskip]: upload to s3:", err.Error(), ", noskip")
		...
		if err == nil {
			...
			if err != nil {
				if strings.ToLower(minio.ToErrorResponse(err).Code) != "nosuchkey" {
					z.Logn("[_cdnskip]: upload to s3:", cnamepath, ", read error:", err.Error())
					return err
				}
			} else {
				if string(bts) == cnametext {
					z.Logn("[_cdnskip]: upload to s3:", cnamepath, ", exists")
					return nil // skip
				} else {
					z.Logn("[_cdnskip]: upload to s3: cname no same", cnamepath, cnametext, string(bts))
				}
			}
		} else if strings.ToLower(minio.ToErrorResponse(err).Code) != "nosuchkey" {
			z.Logn("[_cdnskip]: upload to s3:", cnamepath, ", get object error:", err.Error())
			return err
		}
		z.Logn("[_cdnskip]: upload to s3:", cnamepath, ", noskip")
	}
	...
	z.Logn("[_success]: upload to s3:", cnamepath)
...
func _UploadToS3(ctx context.Context, cli *minio.Client, fpath string, fstat fs.FileInfo, rootdir, rootpath string, //
	...
	if err != nil {
		z.Logn("[_cdn_put_]: upload to s3:", fpath, ", open error:", err.Error())
		return err
	}
	defer file.Close()
	var rbts io.Reader
	...
	if front2.CanFixFileRef(fstat.Name(), ffc) {
		...
		if err != nil {
			z.Logn("[_cdn_put_]: upload to s3:", fpath, ", read error:", err.Error())
			return err
		}
	...
	// z.Logn("[========] upload to s3:", objname, "|", objtype)
	_, err = cli.PutObject(ctx, cfg.Bucket, objname, rbts, size, minio.PutObjectOptions{ContentType: objtype})
	if err != nil {
		z.Logn("[_cdn_put_]: upload to s3:", fpath, ", put error:", err.Error())
		return err
	}
	if isrp {
		z.Logn("[_replace]: upload to s3:", objname)
	} else {
		z.Logn("[_success]: upload to s3:", objname)
	}
	return nil
}
